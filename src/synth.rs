...
mod tests {
    ...
    fn test_g2p_result_multistream() {
        ...
        let result = G2PResult::multistream(
            text_data.clone(),
            t,
            bert_data.clone(),
            duration_extra.clone(),
        );
    ...
    fn test_get_phoneme_ids_multiple() {
        let mut phoneme_id_map = std::collections::HashMap::new();
        phoneme_id_map.insert(
            "а".to_string(),
            crate::model::PhonemeIdValue::Multiple(vec![1, 2]),
        );
    ...
    fn test_bert_tensor_shape_consistency() {
        ...
        // Multistream
        let multistream =
            G2PResult::multistream(vec![0; 5 * t], t, vec![0.0; hidden_size * t], None);
